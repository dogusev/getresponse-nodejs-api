...
function GetResponse(apiKey, print_errors) {
    var apiURL = 'https://api.getresponse360.pl/netpeak';
    ...
    var send = function (data, cb) {
        ...
        request.post(apiURL,options, function (error, response, body) {
                console.log(response.statusCode,error,'this is body',body);
    ...
    this.getCampaigns = function (operator, comparison,cb) {
        ...
        if ($this.textOperators.indexOf(operator)) {
            params = {name: {}};
    ...
    this.getContacts = function ($campaigns, $operator, $comparison, $fields,cb) {
        ...
        $fields.name && (params['name'] = $this.prepTextOp($operator || 'CONTAINS', $comparison || '%'));
        $fields.email && (params['email'] = $this.prepTextOp($operator || 'CONTAINS', $comparison || '%'));
     ...
     * @access private
     */
    this.prepTextOp = function ($operator, $comparison,cb) {
